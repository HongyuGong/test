...
int cond = 4; // default third dimension in tensor
...
int write_chunk(CREC *cr, long long length, FILE *fout) {
    ...
    for (a = 1; a < length; a++) {
        if (cr[a].word1 == old.word1 && cr[a].word2 == old.word2 && cr[a].word3 == old.word3) { // DIFF
            old.val += cr[a].val;
            continue;
        }
...
int compare_crec(const void *a, const void *b) {
    ...
    else if ( (d = ((CREC *) a)->word2 - ((CREC *) b)->word2) != 0 ) return d; // DIFF
...
int compare_crecid(CRECID a, CRECID b) {
    ...
    else if ( (d = a.word2 - b.word2) != 0 ) return d; // DIFF
...
int main(int argc, char **argv) {
    ...
    // tensor: add condition index (z-dimension)
    if ((i = find_arg((char *)"-cond", argc, argv)) > 0) cond = atoi(argv[i + 1]); // DIFF
    if (verbose > 1) fprintf(stderr, "condition index is %d\n", cond); // cond starts from 1
